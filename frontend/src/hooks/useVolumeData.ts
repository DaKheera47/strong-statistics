...
export function useVolumeData(selectedExercises: string[] = []) {
  const [data, setData] = useState<ExerciseVolumeData[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const rawData: VolumeSparklineData[] = await response.json();
        ...
        const grouped = rawData.reduce((acc, row) => {
          ...
          return acc;
        }, {} as Record<string, Array<{date: string, volume: number, sets: number}>>);
        ...
        const transformed = Object.entries(grouped)
          ...
          .map(([exercise, exerciseData]) => {
            ...
            const latestValue = sortedData[sortedData.length - 1]?.volume || 0;
            const previousValue = sortedData[sortedData.length - 2]?.volume || 0;
            
            // Calculate delta based on exercise type
            let delta = 0;
            if (previousValue > 0) {
              if (isLowerBetter(exercise)) {
                // For lower-is-better exercises, improvement is when current < previous (lower volume)
                delta = ((previousValue - latestValue) / previousValue) * 100;
              } else {
                // For normal exercises, improvement is when current > previous (higher volume)
